...
namespace Crawler
{
	...
	public class CrawlerForm : System.Windows.Forms.Form
	{
        ...
        private delegate void SetErrorCountDelegate(string errorCode);
        private SetErrorCountDelegate setErrorCountDelegate;
        private delegate void SetByteCountDelegate(string byteCount);
        private SetByteCountDelegate setByteCountDelegate;
        private delegate void SetCpuStatusDelegate(int cpuStatus, Icon icon);
	    private SetCpuStatusDelegate setCpuStatusDelegate;
        private delegate void SetFileCountDelegate(int fileCount);
	    private SetFileCountDelegate setFileCountDelegate;
		...
		private int ByteCount
		{
			...
			{
				nByteCount = value;
                this.BeginInvoke(setByteCountDelegate, Commas(nByteCount / 1024 + 1) + " KB");
			}
		}
        private void SetByteCount(string byteCount)
        {
            this.statusBarPanelByteCount.Text = byteCount;
        }
		...
		private int ErrorCount
		{
			...
			{
				nErrorCount = value;
                this.BeginInvoke(setErrorCountDelegate, Commas(nErrorCount) + " errors");
			}
		}
        
        private void SetErrorCount(string errorCount)
        {
            this.statusBarPanelErrors.Text = errorCount;
        }
		...
		private int CPUUsage
		{
			...
			set
			{
			    ...
			    try
			    {
                    icon = Icon.FromHandle(((Bitmap)imageListPercentage.Images[value / 10]).GetHicon());
			    }
			    catch (Exception ex)
			    {
			    ...
			    this.BeginInvoke(setCpuStatusDelegate, nCPUUsage, icon);
			}
		}

	    private void SetCpuStatus(int cpuStatus, Icon icon)
	    {
            this.statusBarPanelCPU.Text = "CPU usage " + cpuStatus + "%";
		...
		private int FileCount
		{
			...
			{
				nFileCount = value;
                this.BeginInvoke(setFileCountDelegate, nFileCount);
			}
		}

	    private void SetFileCount(int fileCount)
	    {
            this.statusBarPanelFiles.Text = Commas(fileCount) + " file(s) downloaded";
	    }
		...
		public CrawlerForm()
		{
            ...
            setErrorCountDelegate = SetErrorCount;
            setByteCountDelegate = SetByteCount;
		    setCpuStatusDelegate = SetCpuStatus;
		    setFileCountDelegate = SetFileCount;
	    ...
	    private void LogUrl(string pathName, string pattern, string response, MyUri uri)
	    {
            FileStream stream = null;
            MatchCollection fmatches = new Regex(pattern).Matches(response);
            try
            {
                stream = new FileStream(pathName, FileMode.Append);
                if (fmatches.Count > 0)
                {
                    char[] charUrl = (uri.AbsoluteUri + "\r\n").ToCharArray();
                    Byte[] byUrl = new Byte[charUrl.Length];
                    Encoder e = Encoding.UTF8.GetEncoder();
                    e.GetBytes(charUrl, 0, charUrl.Length, byUrl, 0, true);
                    stream.Seek(0, SeekOrigin.End);
                    stream.Write(byUrl, 0, byUrl.Length);
                }
            }
            catch (Exception e)
            {
	    ...
	    void ParseUri(MyUri uri, ref MyWebRequest request)
		{
			...
			try
			{
				...
				if(ThreadsRunning == true && bParse == true && uri.Depth < WebDepth)
				{
                    ...
                    // parse the page to search for "Facebook"
                    LogUrl(@"C:\facebook.txt", @"MOBI", strResponse, uri);

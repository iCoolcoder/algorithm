 ...
 */
void
MD5_Init(MD5_CTX *context)          /* context */
{
...
MD5_Update( MD5_CTX *context,          /* context */
...
MD5_Final( unsigned char digest[16],  /* message digest */
...
void
MD5_Final_str(char md5_str[], size_t sz, MD5_CTX * md5_ctx)
{
    int i = 0;
    int left_cap = sz;
    ...
    MD5_Final(sum, md5_ctx);

    for(i = 0; i < 16 && left_cap >= 2; i++, left_cap -= 2) {
        md5_str[2 * i] = hex_chars[(sum[i] >> 4) & 0xF];
        md5_str[2 * i + 1] = hex_chars[sum[i] & 0xF];
    }
    if(0 == left_cap)
        left_cap++;
    md5_str[sz - left_cap] = '\0';
}
